param(
    [string]$FirstOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\md-raid1-1.raw'),

    [string]$SecondOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\md-raid1-2.raw'),
...
$generator = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'new-md-raid1-regression-fixture.sh'))
if (-not (Test-Path -LiteralPath $generator -PathType Leaf)) {
    throw "Linux md RAID1 fixture generator was not found: $generator"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Linux md RAID1 fixture generation failed with exit code $LASTEXITCODE."
}
