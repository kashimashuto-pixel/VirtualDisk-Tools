...
printf 'inside subvolume\n' > "$mount_dir/subvol/subvolume.txt"
btrfs subvolume create "$mount_dir/subvol/nested-subvol"
printf 'inside nested subvolume\n' > "$mount_dir/subvol/nested-subvol/nested.txt"
btrfs subvolume snapshot "$mount_dir/subvol" "$mount_dir/snapshot"
subvolume_id="$(btrfs subvolume show "$mount_dir/subvol" | awk '/Subvolume ID:/ { print $3 }')"
test -n "$subvolume_id"
btrfs subvolume set-default "$subvolume_id" "$mount_dir"
...
stat --format='%n size=%s' "$mount_dir/subvol/subvolume.txt" "$mount_dir/subvol/nested-subvol/nested.txt" "$mount_dir/snapshot/subvolume.txt"
sha256sum "$mount_dir/hello.txt" "$mount_dir/nested/regular.bin" "$mount_dir/compressed-zlib.bin" "$mount_dir/compressed-lzo.bin" "$mount_dir/compressed-inline-lzo.bin" "$mount_dir/compressed-zstd.bin" "$mount_dir/compressed-inline-zstd.bin" "$mount_dir/sparse.bin" "$mount_dir/subvol/subvolume.txt" "$mount_dir/subvol/nested-subvol/nested.txt" "$mount_dir/snapshot/subvolume.txt"
