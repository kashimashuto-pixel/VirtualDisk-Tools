param(
    [string]$FirstOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\lvm-striped-1.raw'),

    [string]$SecondOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\lvm-striped-2.raw'),
...
$generator = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'new-lvm-striped-regression-fixture.sh'))
if (-not (Test-Path -LiteralPath $generator -PathType Leaf)) {
    throw "LVM2 striped fixture generator was not found: $generator"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "LVM2 striped fixture generation failed with exit code $LASTEXITCODE."
}
