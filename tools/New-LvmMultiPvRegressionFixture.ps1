param(
    [string]$FirstOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\lvm-multi-pv-1.raw'),

    [string]$SecondOutputPath = (Join-Path $PSScriptRoot '..\.tmp\real-images\lvm-multi-pv-2.raw'),
...
$generator = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'new-lvm-multi-pv-regression-fixture.sh'))
if (-not (Test-Path -LiteralPath $generator -PathType Leaf)) {
    throw "LVM2 multi-PV fixture generator was not found: $generator"
}
...
& wsl.exe `
    --distribution $Distribution `
    --user root `
    --exec bash $linuxGenerator $linuxFirstOutput $linuxSecondOutput $SizeMiB
if ($LASTEXITCODE -ne 0) {
    throw "LVM2 multi-PV fixture generation failed with exit code $LASTEXITCODE."
}
