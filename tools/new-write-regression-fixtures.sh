...
for command_name in truncate mkfs.ext4 e2fsck mkfs.xfs xfs_repair mount mountpoint umount dd sha256sum; do
...
for path in "$ext_image" "$xfs_image" "$replacement"; do
    if [[ -e "$path" ]]; then
...
sha256sum "$ext_image" "$xfs_image" "$replacement" > "$output_dir/source.sha256"
