...
first_output=${1:?usage: new-md-raid0-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
second_output=${2:?usage: new-md-raid0-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
size_mib=${3:?usage: new-md-raid0-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
mount_dir=$(mktemp -d)
first_loop=""
second_loop=""
md_device="/dev/md/vdt-mdraid0-$$"
...
if [[ ! -b "$first_partition" || ! -b "$second_partition" ]]; then
    echo "Linux md RAID0 fixture partition devices were not created." >&2
    exit 4
fi

mkdir -p /dev/md
mdadm --create "$md_device" --run --metadata=1.2 --level=0 --raid-devices=2 \
    --chunk=64 --name=VDT_MDRAID0 "$first_partition" "$second_partition"
mkfs.ext4 -F -L VDT_MDRAID0 "$md_device"
mount "$md_device" "$mount_dir"
printf 'Hello from Linux md RAID0\n' > "$mount_dir/hello.txt"
mkdir "$mount_dir/nested"
printf 'inside md RAID0\n' > "$mount_dir/nested/readme.txt"
...
mdadm --stop "$md_device"
mdadm --examine --export "$first_partition"
mdadm --examine --export "$second_partition"
losetup -d "$first_loop"
first_loop=""
losetup -d "$second_loop"
second_loop=""
