...
first_output=${1:?usage: new-lvm-striped-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
second_output=${2:?usage: new-lvm-striped-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
size_mib=${3:?usage: new-lvm-striped-regression-fixture.sh FIRST_OUTPUT SECOND_OUTPUT SIZE_MIB}
mount_dir=$(mktemp -d)
first_loop=""
second_loop=""
vg_name="VDT_LVM_STRIPED"
lv_name="data"
...
for command_name in truncate sfdisk losetup pvcreate vgcreate vgchange lvcreate lvs mkfs.ext4 e2fsck mount mountpoint umount sha256sum python3; do
...
if [[ ! -b "$first_partition" || ! -b "$second_partition" ]]; then
    echo "LVM2 striped fixture partition devices were not created." >&2
    exit 4
fi
...
vg_created=1
lvcreate --yes --type striped --stripes 2 --stripesize 64 --name "$lv_name" --size 384M \
    "$vg_name" "$first_partition" "$second_partition"
lv_path="/dev/$vg_name/$lv_name"

segment_report=$(lvs --noheadings --separator '|' --segments -o segtype,stripes,stripe_size,devices "$lv_path")
printf '%s\n' "$segment_report"
if ! grep -Fq 'striped|2|' <<<"$segment_report" \
    || ! grep -Fq "$first_partition" <<<"$segment_report" \
    || ! grep -Fq "$second_partition" <<<"$segment_report"; then
    echo "Generated LV is not a two-PV striped segment." >&2
    exit 5
fi

mkfs.ext4 -F -L VDT_LVM_STRIPED "$lv_path"
mount "$lv_path" "$mount_dir"
printf 'Hello from LVM2 striped LV\n' > "$mount_dir/hello.txt"
mkdir "$mount_dir/nested"
printf 'This filesystem crosses 64 KiB LVM stripes.\n' > "$mount_dir/nested/readme.txt"
python3 - "$mount_dir/cross-stripes.bin" <<'PY'
import sys

path = sys.argv[1]
block = bytes(((index * 29 + 7) & 0xff) for index in range(1024 * 1024))
...
lvs --units b --segments -o lv_name,lv_uuid,lv_size,segtype,seg_size,stripes,stripe_size,devices "$lv_path"
vgchange -an "$vg_name"
vg_created=0
losetup -d "$first_loop"
first_loop=""
