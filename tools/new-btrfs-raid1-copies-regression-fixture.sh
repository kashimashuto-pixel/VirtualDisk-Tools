...
profile=${1:?usage: new-btrfs-raid1-copies-regression-fixture.sh PROFILE SIZE_MIB OUTPUT...}
size_mib=${2:?usage: new-btrfs-raid1-copies-regression-fixture.sh PROFILE SIZE_MIB OUTPUT...}
shift 2
outputs=("$@")
mount_dir=$(mktemp -d)
loops=()
...
case "$profile" in
    raid1c3) expected_devices=3 ;;
    raid1c4) expected_devices=4 ;;
    *)
        echo "Unsupported Btrfs profile: $profile" >&2
        exit 2
        ;;
esac
if [[ ${#outputs[@]} -ne $expected_devices ]]; then
    echo "$profile requires exactly $expected_devices output files." >&2
    exit 2
fi
...
mkfs.btrfs -f -m "$profile" -d "$profile" -L "VDT_${profile^^}" "${partitions[@]}"
...
printf 'Hello from Btrfs %s\n' "$profile" > "$mount_dir/hello.txt"
...
printf 'inside %s subvolume\n' "$profile" > "$mount_dir/subvol/subvolume.txt"
sync
